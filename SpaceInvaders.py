        ...
        if bound[0] < width-50:
            for alien in e_all:
                ...
                if alienbounds == None:
                    ...
                    if p.coordinates[1] > alienbounds[1] and p.coordinates[1]< alienbounds[3]:  #Checks Collision with ship
                        game_over()
        ...
        for i in range(len(e_all)-1, -1, -1):
            if bulletshot == True:
                ...
                if bulletBounds == None:
                    ...
                    if i < count:
                        count -= 1
                    c.delete(e_all[i]) #removes alien from array
                    c.delete("shot") #Deletes the bullet
                    e_all.remove(e_all[i])
                    if cheat_code1 == False:
                        speed -= 3
                    score +=100
                    c.itemconfig(Score,text = "Score:" + str(score)) #updates Score
                    if count < 0:
                        L += 1
                        if L == 4:
                            win()
...
class player:
    def __init__(self):
        ...
        self.sprites = PhotoImage(file=r"spaceship.png")
        self.lives = Lives
        def draw(self):
            global ship
...
c.pack()
aliensprite = PhotoImage(file = r"AlienL1.png")
def images():
    img = PhotoImage(file = r"boss.png") 
    window.img = img
